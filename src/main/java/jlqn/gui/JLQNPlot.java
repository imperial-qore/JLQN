...
public class JLQNPlot {
    ...
    static public void plotLQN(LayeredNetwork lqn) throws ExecuteException, IOException {
        ...
        for (int e = 0; e < sn.nentries; e++) {
            entries[e] = Node.builder().shape(NodeShapeEnum.PARALLELOGRAM).color(Color.BLACK).label(sn.names.get(1 + sn.nhosts + sn.ntasks + e)).build();
        }
        ...
        for (int a = 0; a < sn.nacts; a++) {
            Line l = Line
                    .builder(entries[(int) sn.parent.get(1 + sn.nhosts + sn.ntasks + sn.nentries + a)-sn.ntasks-1],acts[a])
                    .arrowHead(ArrowShape.DOT)
                    .build();
            builder = builder.addLine(l);
        }
        for (int c = 0; c < sn.ncalls; c++) {
            Line l = Line
                    .builder(acts[(int) sn.callpair.get(c+1,1)-sn.nhosts -sn.ntasks-sn.nentries-1], entries[(int) sn.callpair.get(c+1,2)-sn.nhosts -sn.ntasks-1]).style(LineStyle.DASHED).arrowHead(ArrowShape.VEE)
                    .build();
            builder = builder.addLine(l);
        }
